...
const ws = new WedSocket.Server({port:8008});

let user_id=0 //주민동록번호처럼 클라이언트에게 부여되는 고유한 값
let ALL_WS = [];// 전체 유저들을 통제할 수 있도록 각 유저에 대한 wevsocket, user_id 저장
ws.on("connection", function connect(websocket,req){ //웹소켓에서 특정 클라이언트가 실행될때 실행문
    user_id++;
